...
from src.shared_utils import NGROK_URL, OrderStatus, get_db_connection, create_tables
import sqlite3
import requests
...
# توكن البوت
KITCHEN_BOT_TOKEN = "7534525770:AAGrBr7Dc1wqqgWHFFOHVO9q-Ff5Bxa-Sso"
...
async def handle_kitchen(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    response = requests.put(f"{NGROK_URL}/api/orders/{order_id}", json={"status": new_status})
...
async def main():
    setup_kitchen_bot()
    await kitchen_application.bot.set_webhook(f"{NGROK_URL}/{KITCHEN_BOT_TOKEN}")
    app.run(port=5001)  # تشغيل على منفذ مختلف عن بوت الزبائن
